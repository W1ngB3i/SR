...
function input(overrides: Partial<typeof baseInput> = {}) {
  keySeq += 1;
  const contact = insertContactKey(dbMod.getDb(), `SM${String(keySeq).padStart(4, '0')}`);
  return { ...baseInput, contact, ...overrides };
}

describe('工单状态机：提交 → 接单 → 回执 → 复核公示 全链路', () => {
  it('提交后进入待接单并返回查询码', () => {
    const result = ticket.createTicket(input(), []);
  ...
  it('接单后进入审核中，时间线记录接单事件', () => {
    const created = ticket.createTicket(input({ circle_name: '接单链路' }), []);
  ...
  it('提交回执缺成绩时按模块报字段错误', () => {
    const created = ticket.createTicket(input({ circle_name: '缺成绩' }), []);
  ...
  it('回执提交后进入已出结果，复核确认后公示', () => {
    const created = ticket.createTicket(input({ circle_name: '全链路' }), []);
    ticket.claimTicket(created.ticket_id, reviewerXingchen);
  ...
  it('复核退回后回到审核中，可重新提交回执', () => {
    const created = ticket.createTicket(input({ circle_name: '复核退回' }), []);
    ticket.claimTicket(created.ticket_id, reviewerXingchen);
    ticket.saveReceipt(
      created.ticket_id,
  ...
  it('退回补充材料后，申请人补交回到审核中', () => {
    const created = ticket.createTicket(input({ circle_name: '补充链路' }), []);
    ticket.claimTicket(created.ticket_id, reviewerXingchen);
  ...
  it('冷却期内同圈名重复提交被拒绝', () => {
    ticket.createTicket(input({ circle_name: '冷却玩家' }), []);
    expect(() => ticket.createTicket(input({ circle_name: '冷却玩家' }), [])).toThrowError(
      /只能提交一条工单/,
    );
  });

  it('非法跳转：待接单直接公示被拒绝', () => {
    const created = ticket.createTicket(input({ circle_name: '非法跳转' }), []);
  ...
  it('非本人无法查询进度', () => {
    const created = ticket.createTicket(input({ circle_name: '查询校验' }), []);
